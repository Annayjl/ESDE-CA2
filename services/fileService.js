...
module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId}   );SELECT @total_records total_records; `;
            ...
            // ' UNION select user_id,fullname,email,user_password from user; SET @total_records =(SELECT count(file_id) FROM file);SELECT @total_records total_records;-- select * from whateveroawdubawodub where '1'='
        }
        //--------------------------------------------------------------------
                ...
                if (err) {
                    ...
                    connection.query(designFileDataQuery, [userId, search, offset, limit], (err, results) => {
                        if (err) {
                            ...
                            //and error.
                            console.log("Yes hello hi it is i", results);
